...
class DNNSortDataset(Dataset):
    def __init__(self):
        ...
        self.similarity = open(config.sort_similarity_path).readlines()
        assert len(self.q_lines) == len(self.sim_q_lines), "DNN Sort中数据长度不一致"
        assert len(self.q_lines) == len(self.similarity), "DNN Sort中数据长度不一致"

    def __getitem__(self, index):
        q = self.q_lines[index].strip().split()
        sim_q = self.sim_q_lines[index].strip().split()
        similarity = int(self.similarity[index].strip())
        ...
        return q, sim_q, similarity, q_length, sim_q_length
...
def collate_fn(batch):
    """
    定义整理函数
    :param batch: [(q, sim_q, similarity, q_length,  sim_q_length), (q, sim_q, similarity, q_length, sim_q_length), ...]
    :return:
    """
    # 对batch依据input_length从大到小排序，只有encoder中需要排序
    batch = sorted(batch, key=lambda x: x[-2], reverse=True)
    # 先对batch拆包，变成多个元组对象，然后从每个元组中取第一个元素组成元组作为zip的第一个元素，相当于矩阵的转置
    q, sim_q, similarity, q_length, sim_q_length = zip(*batch)
    q = torch.LongTensor(q)
    sim_q = torch.LongTensor(sim_q)
    similarity = torch.LongTensor(similarity)
